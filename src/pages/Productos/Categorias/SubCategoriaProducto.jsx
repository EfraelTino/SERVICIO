...
import { getProductoSubCategoria } from "../../../api/productos.api";
import { Link, useParams } from "react-router-dom";
function SubCategoriaProducto() {
    ...
    async function cargarProducto() {
        try {
            const response = await getProductoSubCategoria(categorie, subcategorie)
            const data = response.data;
            console.log(data);
            setSubCategoria(data);
        } catch (error) {
            setErrors(`No se a encontrado productos de ${subcategorie}`)
        }
    }
    useEffect(() => {
        cargarProducto();
                                ...
                                <div className="w-full">
                                    <div className="row my-5 ">
                                        <div className="flex flex-row gap-4">
                                            {subcategoria.map((producto) => (
                                                <div key={producto.id} className="w-1/4 p-5 flex-grow border border-gray200 rounded-lg dark:border-gray">
                                                    <div className="row bg-light p-3 rounded-xl dark:bg-gray mb-3 justify-center flex">
                                                    ...
                                                    <div className="row flex justify-between px-3">
                                                        <div className="flex-grow text-green font-bold text-lg"><p> <span className="text-gray font-normal dark:text-white">Desde: </span> S/{producto.precio_base ? producto.precio_base : 'N/A'}.00</p></div>
